 ...
 *     @desc  : 进程相关工具类
 */
public final class VProcessUtil {

    private VProcessUtil() {
    ...
    public static String getForegroundProcessName() {
        ActivityManager manager = (ActivityManager) VFrame.getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager packageManager = VFrame.getApplication().getPackageManager();
            Intent intent = new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS);
            List<ResolveInfo> list = packageManager.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            System.out.println(list);
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(VFrame.getApplication().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) VFrame.getSystemService(Context.APP_OPS_SERVICE);
                    ...
                    UsageStatsManager usageStatsManager = (UsageStatsManager) VFrame.getSystemService(Context.USAGE_STATS_SERVICE);
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) VFrame.getSystemService(Context.ACTIVITY_SERVICE);
    ...
    public static boolean killBackgroundProcesses(Context context, String packageName) {
        if (VStringUtil.isSpace(packageName)) return false;
        ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
