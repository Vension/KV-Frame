package com.vension.customview.periscopelayout;
...
public class PeriscopeLayout extends RelativeLayout {
	...
	private ValueAnimator getBezierValueAnimator(View target) {
		...
		ValueAnimator animator = ValueAnimator.ofObject(evaluator, new PointF(
				...
				new PointF(random.nextInt(getWidth() > 0 ? getWidth() : 100), 0));
	...
	private PointF getPointF(int scale) {
		...
		pointF.x = random.nextInt((mWidth - 100) > 0 ? (mWidth - 100) : 100);// 减去100 是为了控制 x轴活动范围,看效果 随意~~
		// 再Y轴上 为了确保第二个点 在第一个点之上,我把Y分成了上下两半 这样动画效果好一些 也可以用其他方法
		pointF.y = random.nextInt((mHeight - 100) > 0 ? (mHeight - 100) : 500) / scale;
		return pointF;
	}
